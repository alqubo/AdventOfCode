...
const parsedLines = lines.map(line => line.split(' | ').map(v => v.split(' ')))

let digits = []

parsedLines.forEach(line => {
    const [signals, output] = line
    const counted = output.map(value => ({
        2: 1,
        4: 1,
        7: 1,
        3: 1
...
let total = digits.reduce((acc, v) => acc + v.reduce((acc, v) => acc + v, 0), 0)
...
const displayMap = Object.values(display).map(display => sortString(display))

parsedLines.forEach(line => {
    const [signals, output] = line
    const all = [...output]

    if(all.every(value => displayMap.includes(sortString(value)))) {
        let r = output.map((value) => displayMap.indexOf(sortString(value)));
        console.log(r)
        digits.push(Number(r.join('')))
    }
})

console.log(digits)
total = digits.reduce((acc, v) => acc + v, 0)

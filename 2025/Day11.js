...
// const input2 = demo2.split('\n').map(line => line.trim()).filter(line => line.length > 0)

// Part One
const partOne = () => {
    const graph = input.reduce((acc, curr) => {
        const [node, rest] = curr.split(':');
        acc[node] = rest.split(' ').filter(Boolean);
        return acc
    }, {})
...
const partTwo = () => {
    ...
    const countPaths = (node, req = null , memo = {}, stack = new Set()) => {
        if (node === 'out') {
            return (req === null || req.size === 0) ? 1 : 0;
        }

        if (stack.has(node)) return 0;

        const key = req ? `${node}:${[...req]}` : node;
        if(Object.keys(memo).includes(key)) {
            return memo[key];
        }
        ...
        const next = graph[node] || [];
        const result = next.reduce((acc, curr) => acc + countPaths(curr, newReq, memo, stack), 0);
    ...
    return countPaths("svr", new Set(["dac", "fft"]))
}
